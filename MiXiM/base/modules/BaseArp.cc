...
int BaseArp::getMacAddr(const int netwAddr)
{
    ...
    coreEV << "for host[" << cSimulation::getActiveSimulation()->getModule( netwAddr )->getParentModule()->getIndex()
       << "]: netwAddr " << netwAddr << "; MAC address "
       << cSimulation::getActiveSimulation()->getModule( netwAddr )->getParentModule()->getSubmodule( "nic" )->getId() <<endl;
    return cSimulation::getActiveSimulation()->getModule(netwAddr)->getParentModule()->getSubmodule("nic")->getId();
}
