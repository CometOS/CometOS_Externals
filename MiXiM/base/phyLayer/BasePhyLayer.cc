...
BasePhyLayer::BasePhyLayer() :
	thermalNoise(0), radio(0), decider(0), radioSwitchingOverTimer(0),
			txOverTimer(0), world(0) {
}
...
template double
BasePhyLayer::readPar<double>(const char* parName, const double);
...
void BasePhyLayer::initialize(int stage) {
	...
	if (stage == 0) {
		...
		if (world == NULL) {
			EV << "Could not find BaseWorldUtility module";
		}

	} else if (stage == 1) {
		if (cc->hasPar("sat") && (sensitivity - FWMath::dBm2mW(
				cc->par("sat").doubleValue())) < -0.000001) {
			EV << "Sensitivity can't be smaller than the "
...
void BasePhyLayer::getParametersFromXML(cXMLElement* xmlData,
	...
	for (cXMLElementList::const_iterator it = parameters.begin(); it
			!= parameters.end(); it++) {
		...
		if (name == 0 || type == 0 || value == 0) {
			EV << "Invalid parameter, could not find name, type or value.";
			continue;
		}
...
void BasePhyLayer::finish() {
	...
	decider->finish();
	recordScalar("currChannel",currChannel);
}
...
void BasePhyLayer::initializeDecider(cXMLElement* xmlConfig) {
	...
	if (xmlConfig == 0) {
		EV << "No decider configuration file specified.";
		return;
	}
	...
	if (deciderList.empty()) {
		EV << "No decider configuration found in configuration file.";
		return;
	}

	if (deciderList.size() > 1) {
		EV << "More than one decider configuration found in configuration file.";
		return;
	}
	...
	if (name == 0) {
		EV << "Could not read type of decider from configuration file.";
		return;
	}
	...
	if (decider == 0) {
		EV << "Could not find a decider with the name " << name;
		return;
	}
...
void BasePhyLayer::initializeAnalogueModels(cXMLElement* xmlConfig) {
	...
	if (newAnalogueModel == 0) {
		EV << "Could not find an analogue model with the name " << s.c_str();
	} else {
		analogueModels.push_back(newAnalogueModel);
	}

	if (xmlConfig == 0) {
		EV << "No analogue models configuration file specified.";
		return;
	}
	...
	for (cXMLElementList::const_iterator it = analogueModelList.begin(); it
			!= analogueModelList.end(); it++) {
		...
		if (name == 0) {
			EV << "Could not read name of analogue model.";
			continue;
		}
		...
		AnalogueModel* newAnalogueModel =
				getAnalogueModelFromName(name, params);

		if (newAnalogueModel == 0) {
			EV << 
					"Could not find an analogue model with the name " << name;
			continue;
		}
...
void BasePhyLayer::handleAirFrame(cMessage* msg) {
	...
	if (frame->getRadioChannel() != getCurrentRadioChannel()) {
		delete msg;
		return;
	}
	...
	default:
		EV << "Unknown AirFrame state: " << frame->getState();
		break;
	}
}
...
void BasePhyLayer::handleAirFrameReceiving(AirFrame* frame) {
	...
	if (nextHandleTime < 0) {
		...
		EV << "Invalid next handle time returned by Decider. Expected a value between current simulation time and end of signal but got another one.";
	}
...
AirFrame *BasePhyLayer::encapsMsg(cPacket *macPkt) {
	...
	MacToPhyControlInfo* macToPhyCI =
			static_cast<MacToPhyControlInfo*> (ctrlInfo);
...
void BasePhyLayer::handleChannelSenseRequest(cMessage* msg) {
	...
	if (nextHandleTime >= simTime()) { //schedule request for next handling
		...
		EV << 
				"Next handle time of ChannelSenseRequest returned by the Decider is smaller then current simulation time: " <<
...
void BasePhyLayer::sendMessageDown(AirFrame* msg) {
	msg->setRadioChannel(getCurrentRadioChannel());
	sendToChannel(msg);
}

void  BasePhyLayer::setCurrentRadioChannel(int newRadioChannel) {
	this->currChannel=newRadioChannel;
}
...
void BasePhyLayer::filterSignal(Signal& s) {
	for (AnalogueModelList::const_iterator it = analogueModels.begin(); it
			!= analogueModels.end(); it++) {
...
BasePhyLayer::~BasePhyLayer() {
	...
	//free AnalogueModels
	for (AnalogueModelList::iterator it = analogueModels.begin(); it
			!= analogueModels.end(); it++) {
...
simtime_t BasePhyLayer::setRadioState(int rs) {
	...
	if (txOverTimer->isScheduled()) {
		EV << 
				"Switched radio while sending an AirFrame. The effects this would have on the transmission are not simulated by the BasePhyLayer!";
	}
...
void BasePhyLayer::getChannelInfo(simtime_t from, simtime_t to,
		AirFrameVector& out) {
