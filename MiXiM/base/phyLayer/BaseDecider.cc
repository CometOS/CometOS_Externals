...
simtime_t BaseDecider::handleChannelSenseRequest(ChannelSenseRequest* request) {
	...
	if (currentChannelSenseRequest.first != request) {
		EV << "Got a new ChannelSenseRequest while already handling another one!";
		return notAgain;
	}
...
simtime_t BaseDecider::processSignalHeader(AirFrame* frame) {
    EV << "BaseDecider does not handle Signal headers!";
    return notAgain;
}

simtime_t BaseDecider::processUnknownSignal(AirFrame* frame) {
    EV << "Unknown state for the AirFrame with ID %d", frame->getId();
    return notAgain;
}
