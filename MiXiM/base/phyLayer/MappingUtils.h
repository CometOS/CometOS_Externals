...
public:
	...
	virtual const Argument& getNextPosition() const {
		return origIterator->getNextPosition();
	}
	...
	virtual const Argument& getPosition() const {
		return origIterator->getPosition();
	}

	virtual double getValue() const {
		return origIterator->getValue();
	}
};
 ...
 */
template<
		template<class Key, class Value, class Pair, class Iterator> class Interpolator>
...
protected:
	...
	typedef typename InterpolateableMap<
			simtime_t,
			double,
			Interpolator<simtime_t, double, MapType::value_type,
...
public:
	...
	virtual Mapping* clone() const {
		return new TimeMapping<Interpolator>(*this);
	}
	...
	virtual MappingIterator* createIterator(const Argument& pos) {
		return new TimeMappingIterator<Interpolator>(
				entries.findIntpl(pos.getTime()), isStepMapping);
	}
};
...
public:
	 ...
	 */
	LinearIntplMappingIterator(ConstMappingIterator* leftIt,
			ConstMappingIterator* rightIt, double f);
	 ...
	 */
	virtual const Argument& getNextPosition() const {
		static Argument tmp;// just to suppress warning
		assert(false);
		return tmp;
	}
};
...
public:
	 ...
	 */
	LinearIntplMapping(ConstMapping* left = 0, ConstMapping* right = 0,
			double f = 0.0) :
	...
	virtual MappingIterator* createIterator(const Argument& pos) {
		...
		return new LinearIntplMappingIterator(left->createConstIterator(pos),
				right->createConstIterator(pos), factor);
	}
 ...
 */
template<>
class Linear<double, Mapping*, std::map<double, Mapping*>::value_type
		, std::map<double, Mapping*>::const_iterator> {
public:
...
public:
	 ...
	 * LinearIntplMappings.
	 */
	static double linearInterpolationFactor(const double& t, const double& t0,
			const double& t1) {
		return (t - t0) / (t1 - t0);
	}
	...
	interpolated operator()(const InputIterator& first,
			const InputIterator& last, const double& pos) const {
	 ...
	 */
	interpolated operator()(const InputIterator& first,
			const InputIterator& last, const double& pos,
		...
		return interpolated(
				LinearIntplMapping(left->second, right->second, fact));
	}
};
...
public:
	...
	ConstantSimpleConstMapping(const DimensionSet& dims, const Argument& key,
			double val) :
...
public:
	...
	virtual const Argument& getNextPosition() const {
		return iterator->getNextPosition();
	}
...
public:
	...
	virtual MappingIterator* createIterator(const Argument& pos) {
		return new ConstMappingIteratorWrapper(
				mapping->createConstIterator(pos));
	}
...
protected:
	...
	void updateNextPosition() {
		...
		if (intp || noSubIt || hasNoNext) {
			if (valueIt.hasNext()) {
				ConstMappingIterator* tmp =
						(*valueIt.getNextValue())->createConstIterator();
				nextPosition.setArgValues(tmp->getPosition());
				delete tmp;
			} else {
				nextPosition = position;
			}
			nextPosition.setArgValue(mapping.myDimension,
					valueIt.getNextPosition());
...
public:
	...
	MultiDimMappingIterator(MultiDimMapping<Interpolator>& mapping) :
			mapping(mapping), valueIt(mapping.entries.beginIntpl()), subMapping(
					0), subIterator(0), position() {
	...
	MultiDimMappingIterator(MultiDimMapping<Interpolator>& mapping,
			const Argument& pos) :
			mapping(mapping), valueIt(
					mapping.entries.findIntpl(
							pos.getArgValue(mapping.myDimension))), subMapping(
					0), subIterator(0), position() {
	...
	void jumpTo(const Argument& pos) {
		...
		if (argVal != valueIt.getPosition()
				&& pos.hasArgVal(mapping.myDimension)) {
	 ...
	 */
	virtual void next() {
		if (!subMapping.isInterpolated && subIterator
				&& subIterator->hasNext()) {
			subIterator->next();
		} else {
			valueIt.next();
	...
	virtual bool inRange() const {
		return valueIt.inRange()
				&& (subMapping.isInterpolated
						|| (subIterator && subIterator->inRange()));
	}
	...
	virtual bool hasNext() const {
		return valueIt.hasNext()
				or (subIterator and subIterator->hasNext() and valueIt.inRange());
	}
...
protected:
	...
	MultiDimMapping(const DimensionSet& myDims, Dimension myDim,
			...
			Mapping(myDims), outOfRangeMapping(0), wrappedOORMapping(0), entries(), myDimension(
					myDim), isMaster(false) {
	}
	 ...
	 */
	MultiDimMapping(const DimensionSet& myDims, Dimension myDim,
			ConstantSimpleConstMapping* oorm, ConstMappingWrapper* wrappedoorm,
			InterpolationMethod intpl = STEPS) :
			Mapping(myDims), outOfRangeMapping(oorm), wrappedOORMapping(
					wrappedoorm), entries(wrappedOORMapping), myDimension(
					myDim), isMaster(false) {
	}
	...
	MultiDimMapping(const MultiDimMapping<Interpolator>& o,
			ConstantSimpleConstMapping* oorm, ConstMappingWrapper* wrappedoorm) :
			Mapping(o), outOfRangeMapping(oorm), wrappedOORMapping(wrappedoorm), entries(
					o.entries), myDimension(o.myDimension), isMaster(false) {
	...
	Mapping* createSubSignal() const {
		...
		if (wrappedOORMapping == 0) {
			...
			if (nextDim == Dimension::time)
				return new TimeMapping<Interpolator>(
						outOfRangeMapping->getValue());
			else
				return new MultiDimMapping<Interpolator>(dimensions, nextDim,
						outOfRangeMapping, wrappedOORMapping, LINEAR);
		}
	}
	...
	void copySubMappings(const MultiDimMapping& o) {
		...
		if (nextDim == Dimension::time_static()) {
			for (typename SubFunctionMap::iterator it = entries.begin();
					it != entries.end(); it++) {
				it->second = new TimeMapping<Interpolator>(
						*(static_cast<TimeMapping<Interpolator>*>(it->second)));
			}
		} else {
			for (typename SubFunctionMap::iterator it = entries.begin();
					it != entries.end(); it++) {
				if (outOfRangeMapping == 0) {
					it->second =
							new MultiDimMapping<Interpolator>(
									*(static_cast<MultiDimMapping<Interpolator>*>(it->second)));
				} else {
					it->second =
							new MultiDimMapping<Interpolator>(
									*(static_cast<MultiDimMapping<Interpolator>*>(it->second)),
									outOfRangeMapping, wrappedOORMapping);
				}
			}
		}
	}
...
public:
	 ...
	 */
	MultiDimMapping(const DimensionSet& myDims, InterpolationMethod intpl =
			STEPS) :
			Mapping(myDims), outOfRangeMapping(0), wrappedOORMapping(0), entries(), isMaster(
					true) {
	 ...
	 * Also takes the interpolation-method but is not used yet.
	 */
	MultiDimMapping(const DimensionSet& myDims, double oorv,
			InterpolationMethod intpl = STEPS) :
			Mapping(myDims), outOfRangeMapping(
					new ConstantSimpleConstMapping(myDims, oorv)), wrappedOORMapping(
					new ConstMappingWrapper(outOfRangeMapping)), entries(
					wrappedOORMapping), isMaster(true) {
	 ...
	 */
	MultiDimMapping(const MultiDimMapping<Interpolator>& o) :
			Mapping(o), outOfRangeMapping(o.outOfRangeMapping), wrappedOORMapping(
					o.wrappedOORMapping), entries(o.entries), myDimension(
					o.myDimension), isMaster(true) {

		if (outOfRangeMapping != 0) {
			outOfRangeMapping = new ConstantSimpleConstMapping(dimensions,
					o.outOfRangeMapping->getValue());
	 ...
	 */
	virtual Mapping* clone() const {
		return new MultiDimMapping<Interpolator>(*this);
	}
	...
	virtual void setValue(const Argument& pos, double value) {
		...
		if (posIt == entries.end() || posIt->first != argVal) {
			...
			posIt = entries.insert(posIt,
					typename SubFunctionMap::value_type(argVal, subF));
		}
	...
	void fillRefIfNecessary() {
		...
		for (KeySet::const_iterator keyIt = it->second.begin();
			...
			entries.insert(entries.end(),
					SubFunctionMap::value_type(*keyIt, fillRef));
		}
	}

	FilledUpMapping(const DimensionSet& myDims, Dimension myDim,
			const KeyMap* keys, InterpolationMethod intpl = STEPS) :
			MultiDimMapping<Linear>(myDims, myDim, intpl), fillRef(0), keys(
					keys) {
		fillRefIfNecessary();
	}
...
public:
	FilledUpMapping(ConstMapping* source, const DimensionSet& dims,
			const KeyMap* keys) :
	...
	virtual void appendValue(const Argument& pos, double value) {
		...
		if (fillRef != 0) {
			...
			if (posIt == entries.end() || posIt->first != argVal) {
				Mapping* subF = createSubSignal();
				posIt = entries.insert(posIt,
						SubFunctionMap::value_type(argVal, subF));
			}
...
private:
	static void freeMappingBuffer(ConstMapping* m) {
		MappingBuffer::iterator it = std::find(mappingBuffer.begin(),
				mappingBuffer.end(), m);
	...
	static ConstMapping* createCompatibleMapping(ConstMapping& src,
			ConstMapping& dst);

	static bool iterateToNext(ConstMappingIterator* it1,
...
public:
	...
	static Mapping* applyElementWiseOperator(ConstMapping& f1, ConstMapping& f2,
			const Argument& intvlStart, const Argument& intvlEnd, Operator op) {
	...
	template<class Operator>
	static Mapping* applyElementWiseOperator(ConstMapping& f1, ConstMapping& f2,
			Operator op, double outOfRangeVal = 0.0,
		...
		if (itF1->inRange()
				&& (!itF2->inRange()
						|| itF1->getPosition() < itF2->getPosition())) {
	 ...
	 * that dimension.
	 */
	static double findMax(ConstMapping& m, const Argument& min,
			const Argument& max);
	 ...
	 * that dimension.
	 */
	static double findMin(ConstMapping& m, const Argument& min,
			const Argument& max);
	 ...
	 *
	 */
	static void addDiscontinuity(Mapping* m, const Argument& pos, double value,
...
public:
	ConcatConstMappingIterator(ConstMapping* baseMapping) :
			FilteredConstMappingIterator(baseMapping->createConstIterator()), baseMapping(
					baseMapping) {
	}

	ConcatConstMappingIterator(ConstMapping* baseMapping, const Argument& pos) :
			FilteredConstMappingIterator(baseMapping->createConstIterator(pos)), baseMapping(
					baseMapping) {
	}
...
public:
	...
	ConcatConstMapping(ConstMapping* refMapping, Iterator first, Iterator last,
			bool continueOutOfRange = true, double oorValue = 0.0, Operator op =
					Operator()) :
			ConstMapping(refMapping->getDimensionSet()), refMapping(refMapping), continueOutOfRange(
					continueOutOfRange), oorValue(oorValue), op(op) {
		while (first != last) {
	...
	Mapping* createConcatenatedMapping() {
		...
		Mapping* result = MappingUtils::applyElementWiseOperator(*refMapping,
				**it, op, oorValue, continueOutOfRange);
...
public:
	...
	virtual ConstMappingIterator* createConstIterator() {
		return new ConstDelayedMappingIterator(mapping->createConstIterator(),
				delay);
	}

	virtual ConstMappingIterator* createConstIterator(const Argument& pos) {
		return new ConstDelayedMappingIterator(
				mapping->createConstIterator(delayPosition(pos)), delay);
	}
...
public:
	...
	virtual MappingIterator* createIterator(const Argument& pos) {
		return new DelayedMappingIterator(
				mapping->createIterator(delayPosition(pos)), delay);
	}
};
