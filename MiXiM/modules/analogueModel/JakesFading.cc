	...
	interval(interval)
{
	...
	for (int i = 0; i < fadingPaths; i++) {
		angleOfArrival[i] = cos(cSimulation::getActiveSimulation()->getSystemModule()->uniform(0, M_PI));
		delay[i] = cSimulation::getActiveSimulation()->getSystemModule()->exponential(delayRMS);
	}
}
