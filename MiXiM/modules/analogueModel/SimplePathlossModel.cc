...
//ev no longer working this way
//#define debugEV (ev.isDisabled()||!debug) ? ev : ev << "PhyLayer(SimplePathlossModel): "
#define debugEV EV << "PhyLayer(SimplePathlossModel): "

using namespace omnetpp;

SimplePathlossConstMapping::SimplePathlossConstMapping(
		const DimensionSet& dimensions, SimplePathlossModel* model,
		const double distFactor) :
		SimpleConstMapping(dimensions), distFactor(distFactor), model(model), hasFrequency(
				dimensions.hasDimension(Dimension::frequency_static())) {
}
...
void SimplePathlossModel::filterSignal(Signal& s) {
	...
	double sqrDistance =
			useTorus ?
					myPos.sqrTorusDist(sendersPos, playgroundSize) :
	...
	double distFactor = pow(sqrDistance, -pathLossAlphaHalf)
			/ (16.0 * M_PI * M_PI);
	debugEV << "distance factor is: " << distFactor << endl;

	//is our signal to attenuate defined over frequency?
	bool hasFrequency =
			s.getTransmissionPower()->getDimensionSet().hasDimension(
					Dimension::frequency);
	debugEV << "Signal contains frequency dimension: "
					<< (hasFrequency ? "yes" : "no") << endl;

	const DimensionSet& domain =
			hasFrequency ?
					DimensionSet::timeFreqDomain : DimensionSet::timeDomain;
