...
bool Decider802154Narrow::syncOnSFD(AirFrame* frame) {
	...
	return sfdErrorProbability < cSimulation::getActiveSimulation()->getSystemModule()->uniform(0, 1, 0);
}
...
simtime_t Decider802154Narrow::processSignalEnd(AirFrame* frame)
{
	...
	while(curTime < end) {
		...
		if (noErrors) {
			...
			noErrors = errorProbability < cSimulation::getActiveSimulation()->getSystemModule()->uniform(0, 1);
		}
		if (snr < snirMin)
			snirMin = snr;
